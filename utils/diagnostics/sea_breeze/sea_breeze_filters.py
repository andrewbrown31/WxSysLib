...
import logging
from utils.diagnostics.sea_breeze.sea_breeze_utils import percentile
...
class Mask_Options:
    ...
    def __init__(self):
        ...
        self.thresholds = {
            ...
            "hour_min_lst":9,                         #LST hour must be greater than or equal to 9
            "hour_max_lst":22,                        #LST hour must be less than or equal to 22
...
def process_time_slice(time_slice,**kwargs):
    """
    Wrapper for using filter_2d with map_blocks
    """
...
def filter_2d(ds,angle_ds=None,lsm=None,props_df_output_path=None,output_land_sea_temperature_diff=False,**kwargs):
    ...
    **kwargs
    ...
    if ds.mask.ndim > 2:
        raise ValueError("mask must be a 2D array (of lat and lon). If you have a time dimension, use map_blocks to process each time slice. See process_time_slice function or filter_3d")
    ...
    #Check lat lon dims
    if (("lat" not in ds.dims) | ("lon" not in ds.dims)):
        raise ValueError("ds must have lat and lon dimensions")

    #Get time for data array
    try:
        time = ds.time.values
    except:
        raise ValueError("ds must have a time attribute")
    ...
    #Initialise condition to keep objects
    ...
    if props_df_output_path is None:
        props_df_output_path = "./props_df.csv"
...
def filter_3d(field,threshold="percentile",threshold_value=None,p=95,hourly_change_ds=None,ta=None,vprime=None,lsm=None,angle_ds=None,save_mask=False,filter_out_path=None,props_df_out_path=None,skipna=False,output_chunks=None,**kwargs):
    ...
    **kwargs
    ...
    if props_df_out_path is None:
        props_df_out_path = "./props_df.csv"
...
def land_sea_temperature_diff_rolling_max(ts,lsm,R_km,dy,dx):
    ...
    #Create KDTree for nearest neighbour lookups over the land
    ...
    #Now look up the N closest land points throughout the domain
    ...
    #And the N closest sea points throughout the domain
    ...
    #Set up spatial footprint (radius) for rolling max filter
    ...
    #Get the land and sea temperatures at the nearest land and sea points
    ...
    #Calculate the land-sea temperature difference
