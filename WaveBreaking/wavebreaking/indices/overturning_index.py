...
def calculate_overturnings(
    ...
    **kwargs
    ...
    original_data_len = len(data[kwargs["lon_name"]])
    ...
    for index, series in tqdm(
        ...
        def get_east_west_lat(df):
            try:   
                lat_west = data[kwargs["lat_name"]][
                    contour_index[contour_index.x.eq(df.min_lon-2)].y.values[0]
                ].values
            except:
                lat_west = data[kwargs["lat_name"]][
                    contour_index[contour_index.x.eq(original_data_len-(df.min_lon-2))].y.values[0]
                ].values
            try:
                lat_east = data[kwargs["lat_name"]][
                    contour_index[contour_index.x.eq(df.max_lon+2)].y.values[-1]
                ].values
            except:
                lat_east = data[kwargs["lat_name"]][
                    contour_index[contour_index.x.eq((df.max_lon+2)-original_data_len)].y.values[0]
                ].values
                
            return float(lat_west),float(lat_east)
        ...
        west_lat = pd.DataFrame(
            {"west_lat": [get_east_west_lat(row)[0] for index, row in df_ot.iterrows()]}
        )
        east_lat = pd.DataFrame(
            {"east_lat": [get_east_west_lat(row)[1] for index, row in df_ot.iterrows()]}
        )
        ...
        overturnings.append(
            gpd.GeoDataFrame(
                pd.concat([dates, levels, orientation, west_lat, east_lat], axis=1), geometry=polys
            )
        )
